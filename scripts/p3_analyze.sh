...
#SBATCH -c 64
#SBATCH -o logs/latest_p3_analyze.log
...
# DATASET_TYPES=("glove.6B.50d")
# SIZES=("small" "medium" "big")

RESULT="results/p3_analyze.csv"
...
echo "dataset,num_queries,batch_size,total_time,num_errors,throughput" > $RESULT
...
for dataset_type in "${DATASET_TYPES[@]}"; do
    for size in "${SIZES[@]}"; do
        ...
        apptainer exec \
            ...
            $container \
            bash -c "src/assignment7_problem3.py --dataset $DATASET --queries $QUERIES --labels $LABELS --batch-size $BATCH_SIZE;" > $TMP_LOG
        ...
        echo "$dataset_type,$num_queries,$BATCH_SIZE,$total_time,$errors,$throughput" >> $RESULT
    done
done
