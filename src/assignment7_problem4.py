...
def batch_scan_matrix_mul_cupy(X, Q, b = None):
    """
    Perform linear scan for querying nearest neighbor using cupy.
    X: n*d dataset (in device memory)
    Q: m*d queries (in device memory)
    ...
    """
    m = Q.shape[0]
    I = cp.zeros(m, dtype=np.int64) 
    
    if b is None:
        b = m

    Q_sq_sum = cp.sum(Q ** 2, axis=1)
    X_sq_sum = cp.sum(X ** 2, axis=1)

    X_sq_sum = X_sq_sum[:, cp.newaxis]

    for i in range (0, m, b):
        queries = Q[i : i + b]
        queries_sq_sum = Q_sq_sum[i : i + b]
        batch_size = queries.shape[0]

        XQ_T = cp.dot(X, queries.T)

        distances = X_sq_sum + queries_sq_sum - (2 * XQ_T)

        I[i : i + batch_size] = cp.argmin(distances, axis=0)
...
if __name__ == '__main__':
    ...
    I_device = batch_scan_matrix_mul_cupy(X_dev,Q_dev,args.batch_size)
